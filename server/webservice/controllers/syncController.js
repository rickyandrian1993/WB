...
const updateUploadFlagPccMillYieldActivity = (cd) => {
  ...
  try {
    pool.query(query, (err, _) => {
      if (err) console.error('Error Update Flag Mill Yields', err)
    })
  } catch (error) {
    console.error('Error Update Flag Mill Yields', error)
  }
}
...
const updateFlagEvacActivityDtl = (cd) => {
  ...
  try {
    pool.query(query, (err, _) => {
      if (err) console.error('Error Update Flag Evacuation Activity Detail', err)
    })
  } catch (error) {
    console.error('Error Update Flag Evacuation Activity Detail', error)
  }
}
